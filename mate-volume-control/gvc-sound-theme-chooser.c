...
G_DEFINE_TYPE_WITH_PRIVATE(GvcSoundThemeChooser, gvc_sound_theme_chooser,
                           GTK_TYPE_BOX)
...
static void on_combobox_changed(GtkComboBox *widget,
  ...
  if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(chooser->priv->combo_box),
                                    &iter) == FALSE) {
    return;
  }
  ...
  g_settings_set_string(chooser->priv->sound_settings, SOUND_THEME_KEY,
                        theme_name);

  /* special case for no sounds */
  if (strcmp(theme_name, NO_SOUNDS_THEME_NAME) == 0) {
    g_settings_set_boolean(chooser->priv->sound_settings, EVENT_SOUNDS_KEY,
                           FALSE);
    return;
  } else {
    g_settings_set_boolean(chooser->priv->sound_settings, EVENT_SOUNDS_KEY,
...
static char *load_index_theme_name(const char *index, char **parent) {
  ...
  file = g_key_file_new();
  if (g_key_file_load_from_file(file, index, G_KEY_FILE_KEEP_TRANSLATIONS,
                                NULL) == FALSE) {
  ...
  if (!hidden) {
    indexname =
        g_key_file_get_locale_string(file, "Sound Theme", "Name", NULL, NULL);
...
static void sound_theme_in_dir(GHashTable *hash, const char *dir) {
  GDir *d;
  const char *name;
...
static void add_theme_to_store(const char *key, const char *value,
                               GtkListStore *store) {
  char *parent;
  ...
  gtk_list_store_insert_with_values(store, NULL, G_MAXINT, THEME_DISPLAY_COL,
                                    value, THEME_IDENTIFIER_COL, key,
                                    THEME_PARENT_ID_COL, parent, -1);
  g_free(parent);
}

static void set_combox_for_theme_name(GvcSoundThemeChooser *chooser,
                                      const char *name) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  gboolean found;
   ...
   * one "freedesktop" */
  if (found) {
    gtk_combo_box_set_active_iter(GTK_COMBO_BOX(chooser->priv->combo_box),
                                  &iter);
...
static void set_input_feedback_enabled(GvcSoundThemeChooser *chooser,
                                       gboolean enabled) {
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(chooser->priv->click_feedback_button), enabled);
}

static void setup_theme_selector(GvcSoundThemeChooser *chooser) {
  GHashTable *hash;
  GtkListStore *store;
  ...
  store = gtk_list_store_new(THEME_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING,
                             G_TYPE_STRING);

  /* Add the themes to a combobox */
  gtk_list_store_insert_with_values(
      store, NULL, G_MAXINT, THEME_DISPLAY_COL, _("No sounds"),
      THEME_IDENTIFIER_COL, "__no_sounds", THEME_PARENT_ID_COL, NULL, -1);
...
static xmlChar *xml_get_and_trim_names(xmlNodePtr node) {
  ...
  for (cur = node->children; cur; cur = cur->next) {
    if (!xmlStrcmp(cur->name, GVC_SOUND_NAME)) {
      ...
      if (cur_pri <= keep_pri) {
        if (keep_lang) xmlFree(keep_lang);
...
static void populate_model_from_node(GvcSoundThemeChooser *chooser,
                                     GtkTreeModel *model, xmlNodePtr node) {
  xmlNodePtr child;
  xmlChar *filename;
  xmlChar *name;
  ...
  if (filename != NULL && name != NULL) {
    gtk_list_store_insert_with_values(
        GTK_LIST_STORE(model), NULL, G_MAXINT, ALERT_IDENTIFIER_COL, filename,
        ALERT_DISPLAY_COL, name, ALERT_SOUND_TYPE_COL, _("Built-in"),
        ALERT_ACTIVE_COL, FALSE, -1);
  }
...
static void populate_model_from_file(GvcSoundThemeChooser *chooser,
                                     GtkTreeModel *model,
                                     const char *filename) {
  xmlDocPtr doc;
  xmlNodePtr root;
...
static void populate_model_from_dir(GvcSoundThemeChooser *chooser,
                                    GtkTreeModel *model, const char *dirname) {
  GDir *d;
  const char *name;
...
static gboolean save_alert_sounds(GvcSoundThemeChooser *chooser,
                                  const char *id) {
  ...
  if (utime(path, NULL) != 0) {
    g_warning("Failed to update mtime for directory '%s': %s", path,
              g_strerror(errno));
  }
  g_free(path);
...
static void update_alert_model(GvcSoundThemeChooser *chooser, const char *id) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  do {
    ...
    gtk_tree_model_get(model, &iter, ALERT_IDENTIFIER_COL, &this_id, -1);
    ...
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, ALERT_ACTIVE_COL, toggled,
                       -1);
  } while (gtk_tree_model_iter_next(model, &iter));
}

static void update_alert(GvcSoundThemeChooser *chooser, const char *alert_id) {
  ...
  theme_model =
      gtk_combo_box_get_model(GTK_COMBO_BOX(chooser->priv->combo_box));
  ...
  gtk_tree_model_get(theme_model, &iter, THEME_IDENTIFIER_COL, &theme,
                     THEME_IDENTIFIER_COL, &parent, -1);
  ...
  if (add_custom) {
    gtk_list_store_insert_with_values(GTK_LIST_STORE(theme_model), NULL,
                                      G_MAXINT, THEME_DISPLAY_COL, _("Custom"),
                                      THEME_IDENTIFIER_COL, CUSTOM_THEME_NAME,
                                      THEME_PARENT_ID_COL, theme, -1);
    ...
    do {
      ...
      gtk_tree_model_get(theme_model, &iter, THEME_PARENT_ID_COL, &this_parent,
                         -1);
...
static void on_alert_toggled(GtkCellRendererToggle *renderer, char *path_str,
                             GvcSoundThemeChooser *chooser) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  GtkTreePath *path;
  ...
  id = NULL;
  gtk_tree_model_get(model, &iter, ALERT_IDENTIFIER_COL, &id, ALERT_ACTIVE_COL,
                     &toggled, -1);
...
static void play_preview_for_path(GvcSoundThemeChooser *chooser,
  ...
  gtk_tree_model_get(model, &iter, ALERT_IDENTIFIER_COL, &id, -1);
  if (id == NULL) return;
  ...
  if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(chooser->priv->combo_box),
    ...
    gtk_tree_model_get(theme_model, &theme_iter, THEME_IDENTIFIER_COL,
                       &theme_id, THEME_PARENT_ID_COL, &parent_id, -1);
  ...
  if (strcmp(id, DEFAULT_ALERT_ID) == 0) {
    if (parent_theme != NULL) {
      ca_gtk_play_for_widget(
          GTK_WIDGET(chooser), 0, CA_PROP_APPLICATION_NAME,
          _("Sound Preferences"), CA_PROP_EVENT_ID, "bell-window-system",
          CA_PROP_CANBERRA_XDG_THEME_NAME, parent_theme,
          CA_PROP_EVENT_DESCRIPTION, _("Testing event sound"),
          CA_PROP_CANBERRA_CACHE_CONTROL, "never", CA_PROP_APPLICATION_ID,
      ...
      ca_gtk_play_for_widget(GTK_WIDGET(chooser), 0, CA_PROP_APPLICATION_NAME,
                             _("Sound Preferences"), CA_PROP_EVENT_ID,
                             "bell-window-system", CA_PROP_EVENT_DESCRIPTION,
                             _("Testing event sound"),
    ...
    ca_gtk_play_for_widget(GTK_WIDGET(chooser), 0, CA_PROP_APPLICATION_NAME,
                           _("Sound Preferences"), CA_PROP_MEDIA_FILENAME, id,
                           CA_PROP_EVENT_DESCRIPTION, _("Testing event sound"),
                           CA_PROP_CANBERRA_CACHE_CONTROL, "never",
                           CA_PROP_APPLICATION_ID, "org.mate.VolumeControl",
#ifdef CA_PROP_CANBERRA_ENABLE
...
static void on_treeview_selection_changed(GtkTreeSelection *selection,
                                          GvcSoundThemeChooser *chooser) {
  GList *paths;
  GtkTreeModel *model;
  GtkTreePath *path;

  if (chooser->priv->treeview == NULL) return;
...
static GtkWidget *create_alert_treeview(GvcSoundThemeChooser *chooser) {
  GtkListStore *store;
  GtkWidget *treeview;
  GtkCellRenderer *renderer;
  GtkTreeViewColumn *column;
   ...
   * - sound type
   */
  store = gtk_list_store_new(ALERT_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING,
                             G_TYPE_STRING, G_TYPE_BOOLEAN);

  gtk_list_store_insert_with_values(
      store, NULL, G_MAXINT, ALERT_IDENTIFIER_COL, DEFAULT_ALERT_ID,
      ALERT_DISPLAY_COL, _("Default"), ALERT_SOUND_TYPE_COL, _("From theme"),
      ALERT_ACTIVE_COL, TRUE, -1);
...
static void update_theme(GvcSoundThemeChooser *chooser) {
  char *theme_name;
  ...
  events_enabled =
      g_settings_get_boolean(chooser->priv->sound_settings, EVENT_SOUNDS_KEY);
  if (events_enabled) {
    theme_name =
        g_settings_get_string(chooser->priv->sound_settings, SOUND_THEME_KEY);
  } else {
    theme_name = g_strdup(NO_SOUNDS_THEME_NAME);
  }

  gtk_widget_set_sensitive(chooser->priv->selection_box, events_enabled);
  gtk_widget_set_sensitive(chooser->priv->click_feedback_button,
                           events_enabled);
...
static void gvc_sound_theme_chooser_class_init(
    GvcSoundThemeChooserClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->dispose = gvc_sound_theme_chooser_dispose;
}

static void on_click_feedback_toggled(GtkToggleButton *button,
                                      GvcSoundThemeChooser *chooser) {
  gboolean enabled;

  enabled = gtk_toggle_button_get_active(button);

  g_settings_set_boolean(chooser->priv->sound_settings, INPUT_SOUNDS_KEY,
                         enabled);
}

static void on_key_changed(GSettings *settings, gchar *key,
                           GvcSoundThemeChooser *chooser) {
  if (!strcmp(key, EVENT_SOUNDS_KEY) || !strcmp(key, SOUND_THEME_KEY) ||
      !strcmp(key, INPUT_SOUNDS_KEY))
    update_theme(chooser);
}

static void setup_list_size_constraint(GtkWidget *widget, GtkWidget *to_size) {
  GtkRequisition req;
  gint sc_height;
  int max_height;
  ...
  gdk_window_get_geometry(
      gdk_screen_get_root_window(gtk_widget_get_screen(widget)), NULL, NULL,
      NULL, &sc_height);
...
static void gvc_sound_theme_chooser_init(GvcSoundThemeChooser *chooser) {
  GtkWidget *box;
  GtkWidget *label;
  ...
  gtk_box_pack_start(GTK_BOX(chooser), chooser->priv->theme_box, FALSE, FALSE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(chooser->priv->theme_box),
                     chooser->priv->combo_box, FALSE, FALSE, 6);
  ...
  chooser->priv->click_feedback_button =
      gtk_check_button_new_with_mnemonic(_("Enable _window and button sounds"));
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(chooser->priv->click_feedback_button),
      g_settings_get_boolean(chooser->priv->sound_settings, INPUT_SOUNDS_KEY));
...
GtkWidget *gvc_sound_theme_chooser_new(void) {
  return g_object_new(GVC_TYPE_SOUND_THEME_CHOOSER, "spacing", 6, "orientation",
