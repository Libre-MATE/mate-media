...
static void update_icon_input(GvcStatusIcon *status_icon) {
  ...
  gtk_status_icon_set_visible(GTK_STATUS_ICON(status_icon->priv->icon_input),
                              show);
}

static void update_icon_output(GvcStatusIcon *status_icon) {
  MateMixerStream *stream;
  MateMixerStreamControl *control = NULL;

  stream =
      mate_mixer_context_get_default_output_stream(status_icon->priv->context);
...
static gboolean update_default_input_stream(GvcStatusIcon *status_icon) {
  MateMixerStream *stream;

  stream =
      mate_mixer_context_get_default_input_stream(status_icon->priv->context);
  if (stream == status_icon->priv->input) return FALSE;
...
static void on_context_default_input_stream_notify(MateMixerContext *context,
                                                   GParamSpec *pspec,
                                                   GvcStatusIcon *status_icon) {
  if (update_default_input_stream(status_icon) == FALSE) return;

  update_icon_input(status_icon);
}

static void on_context_default_output_stream_notify(
    MateMixerContext *control, GParamSpec *pspec, GvcStatusIcon *status_icon) {
  update_icon_output(status_icon);
}
...
void gvc_status_icon_start(GvcStatusIcon *status_icon) {
  ...
  if (G_UNLIKELY(status_icon->priv->running == TRUE)) return;

  if (G_UNLIKELY(mate_mixer_context_open(status_icon->priv->context) ==
                 FALSE)) {
...
static void gvc_status_icon_dispose(GObject *object) {
...
static void gvc_status_icon_init(GvcStatusIcon *status_icon) {
  ...
  status_icon->priv->icon_input =
      gvc_stream_status_icon_new(NULL, icon_names_input);
  status_icon->priv->icon_output =
      gvc_stream_status_icon_new(NULL, icon_names_output);

  gvc_stream_status_icon_set_display_name(status_icon->priv->icon_input,
                                          _("Input"));
  gvc_stream_status_icon_set_display_name(status_icon->priv->icon_output,
                                          _("Output"));
  ...
  mate_mixer_context_set_app_id(status_icon->priv->context,
                                GVC_STATUS_ICON_DBUS_NAME);
