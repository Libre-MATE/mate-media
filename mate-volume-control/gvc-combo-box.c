...
enum { COL_NAME, COL_HUMAN_NAME, NUM_COLS };

enum { CHANGING, BUTTON_CLICKED, LAST_SIGNAL };
...
static void on_switch_active_option_notify(MateMixerSwitch *swtch,
                                           GParamSpec *pspec,
  ...
  while (cont == TRUE) {
    ...
    gtk_tree_model_get(combobox->priv->model, &iter, COL_NAME, &current, -1);
...
static void gvc_combo_box_set_switch(GvcComboBox *combobox,
                                     MateMixerSwitch *swtch) {
  MateMixerSwitchOption *active;
  const GList *options;
  ...
  while (options != NULL) {
    ...
    gtk_list_store_insert_with_values(
        GTK_LIST_STORE(combobox->priv->model), &iter, G_MAXINT, COL_NAME,
        mate_mixer_switch_option_get_name(option), COL_HUMAN_NAME,
        mate_mixer_switch_option_get_label(option), -1);
...
static void gvc_combo_box_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_SWITCH:
      ...
      break;
    case PROP_LABEL:
      gtk_label_set_text_with_mnemonic(GTK_LABEL(self->priv->label),
                                       g_value_get_string(value));
      break;
    case PROP_BUTTON_LABEL:
      gtk_button_set_label(GTK_BUTTON(self->priv->button),
                           g_value_get_string(value));
      break;
...
static void gvc_combo_box_get_property(GObject *object, guint prop_id,
...
static void gvc_combo_box_class_init(GvcComboBoxClass *klass) {
  ...
  properties[PROP_SWITCH] = g_param_spec_object(
      "switch", "switch", "The MateMixerSwitch", MATE_MIXER_TYPE_SWITCH,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

  properties[PROP_LABEL] = g_param_spec_string(
      "label", "label", "The combo box label", _("_Profile:"),
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_SHOW_BUTTON] = g_param_spec_boolean(
      "show-button", "show-button", "Whether to show the button", FALSE,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_BUTTON_LABEL] = g_param_spec_string(
      "button-label", "button-label", "The button's label", "",
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  g_object_class_install_properties(object_class, N_PROPERTIES, properties);

  signals[CHANGING] =
      g_signal_new("changing", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GvcComboBoxClass, changing), NULL, NULL,
                   g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1,
                   MATE_MIXER_TYPE_SWITCH_OPTION);

  signals[BUTTON_CLICKED] = g_signal_new(
      "button-clicked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GvcComboBoxClass, button_clicked), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void on_combo_box_changed(GtkComboBox *widget, GvcComboBox *combobox) {
  GtkTreeIter iter;
  gchar *name;
  MateMixerSwitchOption *option;

  if (G_UNLIKELY(gtk_combo_box_get_active_iter(GTK_COMBO_BOX(widget), &iter) ==
                 FALSE))
    return;

  gtk_tree_model_get(combobox->priv->model, &iter, COL_NAME, &name, -1);
...
static void on_combo_box_button_clicked(GtkButton *button,
                                        GvcComboBox *combobox) {
...
static void gvc_combo_box_init(GvcComboBox *combobox) {
  GtkWidget *frame;
  ...
  combobox->priv->model = GTK_TREE_MODEL(
      gtk_list_store_new(NUM_COLS, G_TYPE_STRING, G_TYPE_STRING));

  combobox->priv->combobox =
      gtk_combo_box_new_with_model(combobox->priv->model);
  ...
  gtk_combo_box_set_popup_fixed_width(GTK_COMBO_BOX(combobox->priv->combobox),
                                      FALSE);

  gtk_box_pack_start(GTK_BOX(combobox), frame, TRUE, TRUE, 0);

  gtk_box_pack_start(GTK_BOX(combobox->priv->drop_box),
                     combobox->priv->start_box, FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(combobox->priv->start_box), combobox->priv->label,
                     FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(combobox->priv->drop_box),
                     combobox->priv->combobox, TRUE, TRUE, 0);

  combobox->priv->button = gtk_button_new_with_label("");

  gtk_box_pack_start(GTK_BOX(combobox->priv->drop_box), combobox->priv->button,
                     FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(combobox->priv->drop_box), combobox->priv->end_box,
                     FALSE, FALSE, 0);
...
GtkWidget *gvc_combo_box_new(MateMixerSwitch *swtch, const gchar *label) {
  return g_object_new(GVC_TYPE_COMBO_BOX, "switch", swtch, "label", label,
