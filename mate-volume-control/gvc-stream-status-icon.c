...
G_DEFINE_TYPE_WITH_PRIVATE(GvcStreamStatusIcon, gvc_stream_status_icon,
                           GTK_TYPE_STATUS_ICON)

static gboolean popup_dock(GvcStreamStatusIcon *icon, guint time) {
  GdkRectangle area;
  ...
  if (gtk_status_icon_get_geometry(GTK_STATUS_ICON(icon), &screen, &area,
                                   &orientation) == FALSE) {
...
static gboolean on_status_icon_button_press(GtkStatusIcon *status_icon,
...
static void on_menu_mute_toggled(GtkMenuItem *item, GvcStreamStatusIcon *icon) {
  mate_mixer_stream_control_set_mute(
      icon->priv->control,
      !mate_mixer_stream_control_get_mute(icon->priv->control));
}

static void on_menu_activate_open_volume_control(GtkMenuItem *item,
                                                 GvcStreamStatusIcon *icon) {
  GError *error = NULL;
...
static void on_status_icon_popup_menu(GtkStatusIcon *status_icon, guint button,
  ...
  if (mate_mixer_stream_control_get_mute(icon->priv->control)) {
    ...
    /* Set icon to medium*/
    image = gtk_image_new_from_icon_name(icon->priv->icon_names[2],
                                         GTK_ICON_SIZE_MENU);
  } else {
    label = g_strdup_printf("%s %s", _("Mute"), icon->priv->display_name);
    /* Set icon to muted*/
    image = gtk_image_new_from_icon_name(icon->priv->icon_names[0],
                                         GTK_ICON_SIZE_MENU);
  }
...
static gboolean on_status_icon_scroll_event(GtkStatusIcon *status_icon,
...
static void gvc_icon_release_grab(GvcStreamStatusIcon *icon,
                                  GdkEventButton *event) {
...
static void popdown_dock(GvcStreamStatusIcon *icon) {
  GdkDisplay *display;
...
static void gvc_icon_grab_notify(GvcStreamStatusIcon *icon,
...
static void gvc_stream_status_icon_set_property(GObject *object, guint prop_id,
                                                const GValue *value,
                                                GParamSpec *pspec) {
...
static void gvc_stream_status_icon_get_property(GObject *object, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
  GvcStreamStatusIcon *self = GVC_STREAM_STATUS_ICON(object);
...
static void gvc_stream_status_icon_dispose(GObject *object) {
...
static void on_status_icon_visible_notify(GvcStreamStatusIcon *icon) {
...
static void on_icon_theme_change(GtkSettings *settings, GParamSpec *pspec,
  ...
  gtk_status_icon_set_from_icon_name(
      GTK_STATUS_ICON(icon), icon->priv->icon_names[icon->priv->current_icon]);
}

static void gvc_stream_status_icon_init(GvcStreamStatusIcon *icon) {
  GtkWidget *frame;
  GtkWidget *box;
...
GvcStreamStatusIcon *gvc_stream_status_icon_new(MateMixerStreamControl *control,
  ...
  return g_object_new(GVC_TYPE_STREAM_STATUS_ICON, "control", control,
