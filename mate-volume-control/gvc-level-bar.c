...
static gboolean layout_changed(LevelBarLayout *layout1,
                               LevelBarLayout *layout2) {
  if (layout1->area.x != layout2->area.x) return TRUE;
  if (layout1->area.y != layout2->area.y) return TRUE;
  if (layout1->area.width != layout2->area.width) return TRUE;
  if (layout1->area.height != layout2->area.height) return TRUE;
  ...
  if (!gdk_rgba_equal(&layout1->color_fg, &layout2->color_fg)) return TRUE;
  if (!gdk_rgba_equal(&layout1->color_bg, &layout2->color_bg)) return TRUE;
  if (!gdk_rgba_equal(&layout1->color_dark, &layout2->color_dark)) return TRUE;

  return FALSE;
}

static gdouble fraction_from_adjustment(GvcLevelBar *bar,
...
static gboolean reset_max_peak(GvcLevelBar *bar) {
...
static void bar_calc_layout(GvcLevelBar *bar) {
  gdouble peak_level;
  ...
  if (bar->priv->orientation == GTK_ORIENTATION_VERTICAL) {
    peak_level =
        bar->priv->peak_fraction * (gdouble)bar->priv->layout.area.height;
    max_peak_level =
        bar->priv->max_peak * (gdouble)bar->priv->layout.area.height;
    ...
    peak_level =
        bar->priv->peak_fraction * (gdouble)bar->priv->layout.area.width;
    max_peak_level =
        bar->priv->max_peak * (gdouble)bar->priv->layout.area.width;
  ...
  bar->priv->layout.peak_num =
      (int)(peak_level / (gdouble)bar->priv->layout.delta);
  bar->priv->layout.max_peak_num =
      (int)(max_peak_level / (gdouble)bar->priv->layout.delta);
}

static void update_peak_value(GvcLevelBar *bar) {
  gdouble value;
  LevelBarLayout layout;
  ...
  if (value > bar->priv->max_peak) {
    ...
    if (bar->priv->max_peak_id > 0) g_source_remove(bar->priv->max_peak_id);
...
static void update_rms_value(GvcLevelBar *bar) {
  bar->priv->rms_fraction =
      fraction_from_adjustment(bar, bar->priv->rms_adjustment);
}
...
static void on_peak_adjustment_value_changed(GtkAdjustment *adjustment,
                                             GvcLevelBar *bar) {
  update_peak_value(bar);
}

static void on_rms_adjustment_value_changed(GtkAdjustment *adjustment,
                                            GvcLevelBar *bar) {
...
void gvc_level_bar_set_peak_adjustment(GvcLevelBar *bar,
  ...
  if (bar->priv->peak_adjustment != NULL) {
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(bar->priv->peak_adjustment),
...
void gvc_level_bar_set_rms_adjustment(GvcLevelBar *bar,
  ...
  if (bar->priv->rms_adjustment != NULL) {
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(bar->priv->rms_adjustment),
...
static void gvc_level_bar_set_property(GObject *object, guint prop_id,
...
static void gvc_level_bar_get_property(GObject *object, guint prop_id,
...
static void gvc_level_bar_size_request(GtkWidget *widget,
...
static void gvc_level_bar_get_preferred_width(GtkWidget *widget, gint *minimum,
                                              gint *natural) {
  GtkRequisition requisition;

  gvc_level_bar_size_request(widget, &requisition);

  if (minimum != NULL) *minimum = requisition.width;
  if (natural != NULL) *natural = requisition.width;
}

static void gvc_level_bar_get_preferred_height(GtkWidget *widget, gint *minimum,
                                               gint *natural) {
  GtkRequisition requisition;

  gvc_level_bar_size_request(widget, &requisition);

  if (minimum != NULL) *minimum = requisition.height;
  if (natural != NULL) *natural = requisition.height;
}

static void gvc_level_bar_size_allocate(GtkWidget *widget,
                                        GtkAllocation *allocation) {
  GvcLevelBar *bar;
  ...
  GTK_WIDGET_CLASS(gvc_level_bar_parent_class)
...
static void curved_rectangle(cairo_t *cr, double x0, double y0, double width,
...
static int gvc_level_bar_draw(GtkWidget *widget, cairo_t *cr) {
  GvcLevelBar *bar;
  ...
  if (bar->priv->orientation == GTK_ORIENTATION_VERTICAL) {
    ...
    for (i = 0; i < NUM_BOXES; i++) {
      ...
      curved_rectangle(cr, bar->priv->layout.area.x + 0.5, by + 0.5,
      ...
      if ((bar->priv->layout.max_peak_num - 1) == i) {
        ...
        /* fill foreground */
        cairo_set_source_rgba(cr, bar->priv->layout.color_fg.red,
                              bar->priv->layout.color_fg.green,
                              bar->priv->layout.color_fg.blue, 0.5);
        cairo_fill_preserve(cr);
      } else {
        /* fill background */
        gdk_cairo_set_source_rgba(cr, &bar->priv->layout.color_bg);
    ...
    for (i = 0; i < NUM_BOXES; i++) {
      ...
      curved_rectangle(cr, bx + 0.5, bar->priv->layout.area.y + 0.5,
                       bar->priv->layout.box_width - 1,
                       bar->priv->layout.box_height - 1,
                       bar->priv->layout.box_radius);
...
static void gvc_level_bar_class_init(GvcLevelBarClass *klass) {
  ...
  properties[PROP_ORIENTATION] = g_param_spec_enum(
      "orientation", "Orientation", "The orientation of the bar",
      GTK_TYPE_ORIENTATION, GTK_ORIENTATION_HORIZONTAL,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_PEAK_ADJUSTMENT] = g_param_spec_object(
      "peak-adjustment", "Peak Adjustment",
      "The GtkAdjustment that contains the current peak value",
      GTK_TYPE_ADJUSTMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_RMS_ADJUSTMENT] = g_param_spec_object(
      "rms-adjustment", "RMS Adjustment",
      "The GtkAdjustment that contains the current rms value",
      GTK_TYPE_ADJUSTMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_SCALE] = g_param_spec_int(
      "scale", "Scale", "Scale", 0, G_MAXINT, GVC_LEVEL_SCALE_LINEAR,
...
static void gvc_level_bar_init(GvcLevelBar *bar) {
  ...
  bar->priv->peak_adjustment =
      GTK_ADJUSTMENT(gtk_adjustment_new(0.0, 0.0, 1.0, 0.05, 0.1, 0.1));
  g_object_ref_sink(bar->priv->peak_adjustment);

  g_signal_connect(bar->priv->peak_adjustment, "value-changed",
                   G_CALLBACK(on_peak_adjustment_value_changed), bar);

  bar->priv->rms_adjustment =
      GTK_ADJUSTMENT(gtk_adjustment_new(0.0, 0.0, 1.0, 0.05, 0.1, 0.1));
