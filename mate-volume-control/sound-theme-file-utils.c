...
char *custom_theme_dir_path(const char *child) {
  ...
  if (child == NULL) return g_strdup(dir);
...
static gboolean directory_delete_recursive(GFile *directory, GError **error) {
  ...
  while (success &&
    ...
    if (success) success = g_file_delete(child, NULL, error);
  }
...
static void delete_one_file(const char *sound_name,
...
void delete_disabled_files(const char **sounds) {
  ...
  for (i = 0; sounds[i] != NULL; i++) delete_one_file(sounds[i], ".disabled");
}

static void create_one_file(GFile *file) {
