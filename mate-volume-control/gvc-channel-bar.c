...
static gboolean on_scale_button_release_event(GtkWidget *widget,
                                              GdkEventButton *event,
                                              GvcChannelBar *bar);
static gboolean on_scale_scroll_event(GtkWidget *widget, GdkEventScroll *event,
                                      GvcChannelBar *bar);

G_DEFINE_TYPE_WITH_PRIVATE(GvcChannelBar, gvc_channel_bar, GTK_TYPE_BOX)

static void create_scale_box(GvcChannelBar *bar) {
  ...
  bar->priv->scale =
      gtk_scale_new(bar->priv->orientation, bar->priv->adjustment);
  if (bar->priv->orientation == GTK_ORIENTATION_VERTICAL) {
    gtk_widget_set_size_request(bar->priv->scale, -1, SCALE_SIZE);

    gtk_range_set_inverted(GTK_RANGE(bar->priv->scale), TRUE);

    gtk_box_pack_start(GTK_BOX(bar->priv->scale_box), bar->priv->start_box,
                       FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(bar->priv->start_box), bar->priv->image, FALSE,
                       FALSE, 0);
    gtk_box_pack_start(GTK_BOX(bar->priv->start_box), bar->priv->label, FALSE,
                       FALSE, 0);
    gtk_box_pack_start(GTK_BOX(bar->priv->start_box), bar->priv->high_image,
                       FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(bar->priv->scale_box), bar->priv->scale, TRUE,
                       TRUE, 0);
    gtk_box_pack_start(GTK_BOX(bar->priv->scale_box), bar->priv->end_box, FALSE,
                       FALSE, 0);

    gtk_box_pack_start(GTK_BOX(bar->priv->end_box), bar->priv->low_image, FALSE,
                       FALSE, 0);
    gtk_box_pack_start(GTK_BOX(bar->priv->end_box), bar->priv->mute_button,
                       FALSE, FALSE, 0);
  } else {
    gtk_widget_set_size_request(bar->priv->scale, SCALE_SIZE, -1);

    gtk_box_pack_start(GTK_BOX(bar->priv->scale_box), bar->priv->image, FALSE,
                       FALSE, 0);
    gtk_box_pack_start(GTK_BOX(bar->priv->scale_box), bar->priv->start_box,
                       FALSE, FALSE, 0);

    gtk_box_pack_end(GTK_BOX(bar->priv->start_box), bar->priv->low_image, FALSE,
                     FALSE, 0);

    gtk_box_pack_start(GTK_BOX(bar->priv->start_box), bar->priv->label, TRUE,
                       TRUE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(bar->priv->end_box), bar->priv->high_image,
                       FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(bar->priv->end_box), bar->priv->mute_button,
                       FALSE, FALSE, 0);
  }
  ...
  if (bar->priv->size_group != NULL) {
    gtk_size_group_add_widget(bar->priv->size_group, bar->priv->start_box);

    if (bar->priv->symmetric)
      gtk_size_group_add_widget(bar->priv->size_group, bar->priv->end_box);
  }
...
static void on_adjustment_value_changed(GtkAdjustment *adjustment,
  ...
  if (bar->priv->control == NULL || bar->priv->click_lock == TRUE) return;
...
static void on_mute_button_toggled(GtkToggleButton *button,
                                   GvcChannelBar *bar) {
  gboolean mute;
...
static void update_layout(GvcChannelBar *bar) {
  GtkWidget *frame;

  if (bar->priv->scale == NULL) return;
  ...
  gtk_container_remove(GTK_CONTAINER(bar->priv->scale_box),
                       bar->priv->start_box);
  gtk_container_remove(GTK_CONTAINER(bar->priv->scale_box), bar->priv->scale);
  gtk_container_remove(GTK_CONTAINER(bar->priv->scale_box), bar->priv->end_box);
  gtk_container_remove(GTK_CONTAINER(frame), bar->priv->scale_box);
...
static void update_marks(GvcChannelBar *bar) {
  gdouble base;
  gdouble normal;
  ...
  if (bar->priv->control == NULL || bar->priv->show_marks == FALSE) return;
  ...
  if (normal <= gtk_adjustment_get_lower(bar->priv->adjustment)) return;

  if (base < normal) {
    gchar *str =
        g_strdup_printf("<small>%s</small>", C_("volume", "Unamplified"));

    gtk_scale_add_mark(GTK_SCALE(bar->priv->scale), base, GTK_POS_BOTTOM, str);
    has_mark = TRUE;
    g_free(str);
  }
  ...
  if (bar->priv->extended && (base == normal || base < normal)) {
    ...
    gtk_scale_add_mark(GTK_SCALE(bar->priv->scale), normal, GTK_POS_BOTTOM,
                       str);
    has_mark = TRUE;
    g_free(str);
  }
...
static void update_adjustment_value(GvcChannelBar *bar) {
  gdouble value;
  ...
  g_signal_handlers_block_by_func(G_OBJECT(bar->priv->adjustment),
                                  on_adjustment_value_changed, bar);
...
static void update_adjustment_limits(GvcChannelBar *bar) {
  gdouble minimum = 0.0;
  gdouble maximum = 0.0;
  ...
  gtk_adjustment_configure(bar->priv->adjustment,
                           ...
                           minimum, maximum, (maximum - minimum) / 100.0,
                           (maximum - minimum) / 15.0, 0.0);
}

static void update_mute_button(GvcChannelBar *bar) {
...
static gboolean on_scale_button_release_event(GtkWidget *widget,
  ...
  ca_gtk_play_for_widget(
      GTK_WIDGET(bar), 0, CA_PROP_EVENT_ID, "audio-volume-change",
      CA_PROP_EVENT_DESCRIPTION, "Volume change", CA_PROP_APPLICATION_ID,
      "org.mate.VolumeControl", CA_PROP_APPLICATION_NAME, _("Volume Control"),
      CA_PROP_APPLICATION_VERSION, VERSION, CA_PROP_APPLICATION_ICON_NAME,
      "multimedia-volume-control", NULL);
  return FALSE;
}
...
static void on_control_volume_notify(MateMixerStreamControl *control,
                                     GParamSpec *pspec, GvcChannelBar *bar) {
  update_adjustment_value(bar);
}

static void on_control_mute_notify(MateMixerStreamControl *control,
                                   GParamSpec *pspec, GvcChannelBar *bar) {
  if (bar->priv->show_mute == TRUE) {
    gboolean mute = mate_mixer_stream_control_get_mute(control);

    g_signal_handlers_block_by_func(G_OBJECT(bar->priv->mute_button),
                                    on_mute_button_toggled, bar);
...
void gvc_channel_bar_set_control(GvcChannelBar *bar,
  ...
  if (bar->priv->control == control) return;

  if (control != NULL) g_object_ref(control);
  ...
  if (bar->priv->control != NULL) {
    ...
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(bar->priv->control), G_CALLBACK(on_control_mute_notify), bar);
...
void gvc_channel_bar_set_orientation(GvcChannelBar *bar,
  ...
  if (orientation == bar->priv->orientation) return;
...
void gvc_channel_bar_set_show_icons(GvcChannelBar *bar, gboolean show_icons) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));

  if (show_icons == bar->priv->show_icons) return;
...
void gvc_channel_bar_set_show_mute(GvcChannelBar *bar, gboolean show_mute) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));

  if (show_mute == bar->priv->show_mute) return;
...
void gvc_channel_bar_set_show_marks(GvcChannelBar *bar, gboolean show_marks) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));

  if (show_marks == bar->priv->show_marks) return;
...
void gvc_channel_bar_set_extended(GvcChannelBar *bar, gboolean extended) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));

  if (extended == bar->priv->extended) return;
...
void gvc_channel_bar_set_name(GvcChannelBar *bar, const gchar *name) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));
...
void gvc_channel_bar_set_icon_name(GvcChannelBar *bar, const gchar *name) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));
  ...
  if (name != NULL) {
    ...
    pixbuf = gtk_icon_theme_load_icon(
        ...
        GTK_ICON_LOOKUP_GENERIC_FALLBACK | GTK_ICON_LOOKUP_FORCE_SIZE, &error);
    if (error != NULL) {
    ...
    if (pixbuf == NULL) {
      pixbuf =
          gdk_pixbuf_new_from_file_at_scale(name, width, height, TRUE, &error);
      if (error != NULL) {
        g_warning("Couldn’t load icon: %s\n", error->message);
        g_clear_error(&error);
      }
...
void gvc_channel_bar_set_low_icon_name(GvcChannelBar *bar, const gchar *name) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));

  gtk_image_set_from_icon_name(GTK_IMAGE(bar->priv->low_image), name,
...
void gvc_channel_bar_set_high_icon_name(GvcChannelBar *bar, const gchar *name) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));

  gtk_image_set_from_icon_name(GTK_IMAGE(bar->priv->high_image), name,
                               GTK_ICON_SIZE_BUTTON);
...
void gvc_channel_bar_set_size_group(GvcChannelBar *bar, GtkSizeGroup *group,
                                    gboolean symmetric) {
  g_return_if_fail(GVC_IS_CHANNEL_BAR(bar));
...
static void gvc_channel_bar_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
...
static void gvc_channel_bar_get_property(GObject *object, guint prop_id,
...
static void gvc_channel_bar_class_init(GvcChannelBarClass *klass) {
  ...
  properties[PROP_CONTROL] = g_param_spec_object(
      "control", "Control", "MateMixer stream control",
      MATE_MIXER_TYPE_STREAM_CONTROL,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_ORIENTATION] = g_param_spec_enum(
      "orientation", "Orientation", "The orientation of the scale",
      GTK_TYPE_ORIENTATION, GTK_ORIENTATION_VERTICAL,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_SHOW_MUTE] = g_param_spec_boolean(
      "show-mute", "show mute", "Whether stream is muted", FALSE,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_SHOW_ICONS] = g_param_spec_boolean(
      "show-icons", "show mute", "Whether to show low and high icons", FALSE,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_SHOW_MARKS] = g_param_spec_boolean(
      "show-marks", "Show marks", "Whether to show scale marks", FALSE,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_EXTENDED] =
      g_param_spec_boolean("extended", "Extended",
                           "Allow the scale to be extended above normal volume",
                           FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_NAME] = g_param_spec_string(
      "name", "Name", "Name to display for this stream", NULL,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_ICON_NAME] = g_param_spec_string(
      "icon-name", "Icon name", "Name of icon to display for this stream", NULL,
      G_PARAM_WRITABLE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_LOW_ICON_NAME] = g_param_spec_string(
      "low-icon-name", "Low icon name",
      "Name of low volume icon to display for this stream", "audio-volume-low",
      G_PARAM_WRITABLE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_HIGH_ICON_NAME] = g_param_spec_string(
      "high-icon-name", "High icon name",
...
static void gvc_channel_bar_init(GvcChannelBar *bar) {
  ...
  g_signal_connect(bar->priv->mute_button, "toggled",
                   G_CALLBACK(on_mute_button_toggled), bar);
  ...
  g_signal_connect(bar->priv->adjustment, "value-changed",
...
GtkWidget *gvc_channel_bar_new(MateMixerStreamControl *control) {
  return g_object_new(GVC_TYPE_CHANNEL_BAR, "control", control, "orientation",
