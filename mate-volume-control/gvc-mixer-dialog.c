...
static const guint tab_accel_keys[] = {GDK_KEY_1, GDK_KEY_2, GDK_KEY_3,
                                       GDK_KEY_4, GDK_KEY_5};

static void gvc_mixer_dialog_finalize(GObject *object);

static void add_stream(GvcMixerDialog *dialog, MateMixerStream *stream);
static void add_application_control(GvcMixerDialog *dialog,
                                    MateMixerStreamControl *control);

static void remove_stream(GvcMixerDialog *dialog, const gchar *name);
static void remove_application_control(GvcMixerDialog *dialog,
                                       const gchar *name);

static void bar_set_stream(GvcMixerDialog *dialog, GtkWidget *bar,
                           MateMixerStream *stream);
static void bar_set_stream_control(GvcMixerDialog *dialog, GtkWidget *bar,
                                   MateMixerStreamControl *control);

G_DEFINE_TYPE_WITH_PRIVATE(GvcMixerDialog, gvc_mixer_dialog, GTK_TYPE_DIALOG)

static MateMixerSwitch *find_stream_port_switch(MateMixerStream *stream) {
  ...
  while (switches != NULL) {
    ...
    if (!MATE_MIXER_IS_STREAM_TOGGLE(swtch) &&
        mate_mixer_stream_switch_get_role(swtch) ==
...
static MateMixerSwitch *find_device_profile_switch(MateMixerDevice *device) {
  ...
  while (switches != NULL) {
    ...
    if (mate_mixer_device_switch_get_role(swtch) ==
...
static MateMixerStream *find_device_test_stream(GvcMixerDialog *dialog,
...
static gboolean find_tree_item_by_name(GtkTreeModel *model, const gchar *name,
                                       guint column, GtkTreeIter *iter) {
  ...
  do {
    ...
    if (!g_strcmp0(name, n)) found = TRUE;
...
static void update_default_tree_item(GvcMixerDialog *dialog,
                                     GtkTreeModel *model,
                                     MateMixerStream *stream) {
  GtkTreeIter iter;
  ...
  if (stream != NULL) name = mate_mixer_stream_get_name(stream);

  do {
    gchar *n;
    gtk_tree_model_get(model, &iter, NAME_COLUMN, &n, -1);
...
static void update_output_settings(GvcMixerDialog *dialog) {
  MateMixerStream *stream;
  MateMixerStreamControl *control;
  ...
  control =
      gvc_channel_bar_get_control(GVC_CHANNEL_BAR(dialog->priv->output_bar));
  if (control == NULL) {
  ...
  if (flags & MATE_MIXER_STREAM_CONTROL_CAN_BALANCE) {
    ...
    gvc_balance_bar_set_size_group(
        GVC_BALANCE_BAR(dialog->priv->output_balance_bar),
        dialog->priv->size_group, TRUE);

    gtk_box_pack_start(GTK_BOX(dialog->priv->output_settings_box),
                       dialog->priv->output_balance_bar, FALSE, FALSE, 6);
  ...
  if (flags & MATE_MIXER_STREAM_CONTROL_CAN_FADE) {
    ...
    gvc_balance_bar_set_size_group(
        GVC_BALANCE_BAR(dialog->priv->output_fade_bar),
        dialog->priv->size_group, TRUE);

    gtk_box_pack_start(GTK_BOX(dialog->priv->output_settings_box),
                       dialog->priv->output_fade_bar, FALSE, FALSE, 6);
  ...
  if (mate_mixer_stream_control_has_channel_position(control,
                                                     MATE_MIXER_CHANNEL_LFE)) {
    dialog->priv->output_lfe_bar =
        gvc_balance_bar_new(control, BALANCE_TYPE_LFE);

    gvc_balance_bar_set_size_group(
        GVC_BALANCE_BAR(dialog->priv->output_lfe_bar), dialog->priv->size_group,
        TRUE);

    gtk_box_pack_start(GTK_BOX(dialog->priv->output_settings_box),
                       dialog->priv->output_lfe_bar, FALSE, FALSE, 6);
  ...
  if (port_switch != NULL) {
    ...
    gtk_box_pack_start(GTK_BOX(dialog->priv->output_settings_box),
                       dialog->priv->output_port_combo, TRUE, FALSE, 6);
...
static void set_output_stream(GvcMixerDialog *dialog, MateMixerStream *stream) {
  GtkTreeModel *model;
  MateMixerStreamControl *control;
...
static void on_context_default_output_stream_notify(MateMixerContext *context,
                                                    GParamSpec *pspec,
...
static void on_stream_control_monitor_value(MateMixerStream *stream,
                                            gdouble value,
                                            GvcMixerDialog *dialog) {
  GtkAdjustment *adj;
  ...
  adj = gvc_level_bar_get_peak_adjustment(
      GVC_LEVEL_BAR(dialog->priv->input_level_bar));
  if (value >= 0)
...
static void update_input_settings(GvcMixerDialog *dialog) {
  MateMixerStream *stream;
  MateMixerStreamControl *control;
  MateMixerStreamControlFlags flags;
  MateMixerSwitch *port_switch;
  ...
  if (port_switch != NULL) {
    ...
    gtk_box_pack_start(GTK_BOX(dialog->priv->input_settings_box),
                       dialog->priv->input_port_combo, TRUE, TRUE, 0);

    gtk_widget_show(dialog->priv->input_port_combo);
  }
}

static void on_stream_control_mute_notify(MateMixerStreamControl *control,
                                          GParamSpec *pspec,
                                          GvcMixerDialog *dialog) {
...
static void set_input_stream(GvcMixerDialog *dialog, MateMixerStream *stream) {
  GtkTreeModel *model;
  MateMixerStreamControl *control;
  ...
  if (control != NULL) {
    ...
    if (dialog->priv->input_port_combo != NULL) {
      ...
      swtch =
          g_object_get_data(G_OBJECT(dialog->priv->input_port_combo), "switch");
      if (swtch != NULL)
        g_signal_handlers_disconnect_by_data(G_OBJECT(swtch), dialog);
    }
    ...
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(control), G_CALLBACK(on_stream_control_monitor_value), dialog);
  ...
  if (stream != NULL) {
    const GList *controls;
    guint page =
        gtk_notebook_get_current_page(GTK_NOTEBOOK(dialog->priv->notebook));
...
static void on_context_default_input_stream_notify(MateMixerContext *context,
                                                   GParamSpec *pspec,
                                                   GvcMixerDialog *dialog) {
  MateMixerStream *stream;
...
static GtkWidget *create_bar(GvcMixerDialog *dialog, gboolean use_size_group,
                             gboolean symmetric) {
  GtkWidget *bar;
  ...
  if (use_size_group == TRUE)
    gvc_channel_bar_set_size_group(GVC_CHANNEL_BAR(bar),
                                   dialog->priv->size_group, symmetric);

  g_object_set(G_OBJECT(bar), "orientation", GTK_ORIENTATION_HORIZONTAL,
               "show-mute", TRUE, "show-icons", TRUE, "show-marks", TRUE,
               "extended", TRUE, NULL);
  return bar;
}
...
static void bar_set_stream_control(GvcMixerDialog *dialog, GtkWidget *bar,
  ...
  if (previous != NULL) {
    ...
    g_debug("Removing stream control %s from bar %s", name,
  ...
  if (control != NULL) {
    ...
    g_debug("Setting stream control %s for bar %s", name,
            gvc_channel_bar_get_name(GVC_CHANNEL_BAR(bar)));
...
static void add_application_control(GvcMixerDialog *dialog,
  ...
  if (app_name == NULL) app_name = mate_mixer_stream_control_get_label(control);
  if (app_name == NULL) app_name = mate_mixer_stream_control_get_name(control);
  if (G_UNLIKELY(app_name == NULL)) return;

  bar = create_bar(dialog, FALSE, FALSE);

  g_object_set(G_OBJECT(bar), "show-marks", FALSE, "extended", FALSE, NULL);

  /* By default channel bars use speaker icons, use microphone icons
   * instead for recording applications */
  stream = mate_mixer_stream_control_get_stream(control);
  if (stream != NULL) direction = mate_mixer_stream_get_direction(stream);

  if (direction == MATE_MIXER_DIRECTION_INPUT)
    g_object_set(G_OBJECT(bar), "low-icon-name", "audio-input-microphone-low",
                 "high-icon-name", "audio-input-microphone-high", NULL);
  ...
  gtk_box_pack_start(GTK_BOX(dialog->priv->applications_box), bar, FALSE, FALSE,
                     12);
...
static void on_stream_control_added(MateMixerStream *stream, const gchar *name,
                                    GvcMixerDialog *dialog) {
  MateMixerStreamControl *control;
  MateMixerStreamControlRole role;

  control = mate_mixer_stream_get_control(stream, name);
  if (G_UNLIKELY(control == NULL)) return;
...
static void on_stream_control_removed(MateMixerStream *stream,
                                      const gchar *name,
                                      GvcMixerDialog *dialog) {
  MateMixerStreamControl *control;
...
static void add_stream(GvcMixerDialog *dialog, MateMixerStream *stream) {
  ...
  if (direction == MATE_MIXER_DIRECTION_INPUT) {
    ...
    model =
        gtk_tree_view_get_model(GTK_TREE_VIEW(dialog->priv->input_treeview));
  } else if (direction == MATE_MIXER_DIRECTION_OUTPUT) {
    MateMixerStream *output;
    MateMixerStreamControl *control;

    output =
        mate_mixer_context_get_default_output_stream(dialog->priv->context);
    if (stream == output) {
      bar_set_stream(dialog, dialog->priv->output_bar, stream);
    ...
    model =
        gtk_tree_view_get_model(GTK_TREE_VIEW(dialog->priv->output_treeview));
  ...
  if (model != NULL) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, NAME_COLUMN, name,
                       LABEL_COLUMN, label, ACTIVE_COLUMN, is_default,
                       SPEAKERS_COLUMN, speakers, -1);
  }
...
static void update_device_test_visibility(GvcMixerDialog *dialog) {
  MateMixerDevice *device;
  MateMixerStream *stream;

  device =
      g_object_get_data(G_OBJECT(dialog->priv->hw_profile_combo), "device");
  ...
  g_object_set(G_OBJECT(dialog->priv->hw_profile_combo), "show-button",
               (stream != NULL), NULL);
}

static void on_context_stream_added(MateMixerContext *context,
                                    const gchar *name, GvcMixerDialog *dialog) {
  MateMixerStream *stream;
  MateMixerDirection direction;
  GtkWidget *bar;
  ...
  if (dialog->priv->hw_profile_combo != NULL &&
    ...
    if (device1 == device2) {
      ...
      g_object_get(G_OBJECT(dialog->priv->hw_profile_combo), "show-button",
                   &show_button, NULL);

      if (show_button == FALSE) update_device_test_visibility(dialog);
    }
  }

  bar = g_hash_table_lookup(dialog->priv->bars, name);
  if (G_UNLIKELY(bar != NULL)) return;
...
static void remove_stream(GvcMixerDialog *dialog, const gchar *name) {
  ...
  if (bar != NULL) {
    g_debug("Removing stream %s from bar %s", name,
            gvc_channel_bar_get_name(GVC_CHANNEL_BAR(bar)));
  ...
  model = gtk_tree_view_get_model(GTK_TREE_VIEW(dialog->priv->output_treeview));
  if (find_tree_item_by_name(GTK_TREE_MODEL(model), name, NAME_COLUMN, &iter) ==
      TRUE)
...
static void remove_application_control(GvcMixerDialog *dialog,
  ...
  if (dialog->priv->num_apps == 0) gtk_widget_show(dialog->priv->no_apps_label);
}
...
static void on_context_stream_removed(MateMixerContext *context,
  ...
  if (dialog->priv->hw_profile_combo != NULL) {
    ...
    if (show_button == TRUE) update_device_test_visibility(dialog);
  }

  remove_stream(dialog, name);
}

static void on_context_stored_control_added(MateMixerContext *context,
                                            const gchar *name,
                                            GvcMixerDialog *dialog) {
  MateMixerStreamControl *control;
  MateMixerStreamControlMediaRole media_role;

  control = MATE_MIXER_STREAM_CONTROL(
      mate_mixer_context_get_stored_control(context, name));
  if (G_UNLIKELY(control == NULL)) return;
...
static void on_context_stored_control_removed(MateMixerContext *context,
                                              const gchar *name,
                                              GvcMixerDialog *dialog) {
  GtkWidget *bar;
...
static gchar *device_status(MateMixerDevice *device) {
  guint inputs = 0;
  guint outputs = 0;
  ...
  if (outputs > 0) {
    ...
    outputs_str =
        g_strdup_printf(ngettext("%u Output", "%u Outputs", outputs), outputs);
  }
  ...
  if (inputs > 0) {
    ...
    inputs_str =
        g_strdup_printf(ngettext("%u Input", "%u Inputs", inputs), inputs);
  }

  if (inputs_str != NULL && outputs_str != NULL) {
    gchar *ret = g_strdup_printf("%s / %s", outputs_str, inputs_str);
    g_free(outputs_str);
    g_free(inputs_str);
    return ret;
  }

  if (inputs_str != NULL) return inputs_str;

  return outputs_str;
}

static void update_device_info(GvcMixerDialog *dialog,
                               MateMixerDevice *device) {
  GtkTreeModel *model = NULL;
  GtkTreeIter iter;
  const gchar *label;
  ...
  if (find_tree_item_by_name(model, mate_mixer_device_get_name(device),
                             HW_NAME_COLUMN, &iter) == FALSE)
    return;
  ...
  gtk_list_store_set(GTK_LIST_STORE(model), &iter, HW_LABEL_COLUMN, label,
                     HW_PROFILE_COLUMN, profile_label, HW_STATUS_COLUMN, status,
                     -1);
  g_free(status);
}

static void on_device_profile_active_option_notify(MateMixerDeviceSwitch *swtch,
                                                   GParamSpec *pspec,
                                                   GvcMixerDialog *dialog) {
  MateMixerDevice *device;
...
static void add_device(GvcMixerDialog *dialog, MateMixerDevice *device) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  GIcon *icon;
  const gchar *name;
  ...
  if (find_tree_item_by_name(GTK_TREE_MODEL(model), name, HW_NAME_COLUMN,
                             &iter) == FALSE)
    gtk_list_store_append(GTK_LIST_STORE(model), &iter);
  ...
  gtk_list_store_set(GTK_LIST_STORE(model), &iter, HW_NAME_COLUMN, name,
                     HW_LABEL_COLUMN, label, HW_ICON_COLUMN, icon,
                     HW_PROFILE_COLUMN, profile_label, HW_STATUS_COLUMN, status,
                     -1);
  g_free(status);
}
...
static void on_input_radio_toggled(GtkCellRendererToggle *renderer,
                                   gchar *path_str, GvcMixerDialog *dialog) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  GtkTreePath *path;
  ...
  gtk_tree_model_get(model, &iter, NAME_COLUMN, &name, ACTIVE_COLUMN, &toggled,
                     -1);
  if (toggled ^ 1) {
    MateMixerStream *stream;
    ...
    if (flags & MATE_MIXER_BACKEND_CAN_SET_DEFAULT_INPUT_STREAM)
      mate_mixer_context_set_default_input_stream(dialog->priv->context,
                                                  stream);
    else
      set_input_stream(dialog, stream);
  }
...
static void on_output_radio_toggled(GtkCellRendererToggle *renderer,
                                    gchar *path_str, GvcMixerDialog *dialog) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  GtkTreePath *path;
  ...
  if (toggled ^ 1) {
    ...
    if (flags & MATE_MIXER_BACKEND_CAN_SET_DEFAULT_OUTPUT_STREAM)
      mate_mixer_context_set_default_output_stream(dialog->priv->context,
                                                   stream);
    else
      set_output_stream(dialog, stream);
  }
...
static void stream_name_to_text(GtkTreeViewColumn *column,
                                GtkCellRenderer *cell, GtkTreeModel *model,
                                GtkTreeIter *iter, gpointer user_data) {
  gchar *label;
  gchar *speakers;

  gtk_tree_model_get(model, iter, LABEL_COLUMN, &label, SPEAKERS_COLUMN,
                     &speakers, -1);

  if (speakers != NULL) {
    gchar *str = g_strdup_printf("%s\n<i>%s</i>", label, speakers);
...
static gint compare_stream_treeview_items(GtkTreeModel *model, GtkTreeIter *a,
                                          GtkTreeIter *b, gpointer user_data) {
  gchar *desc_a = NULL;
  gchar *desc_b = NULL;
  gint result;

  gtk_tree_model_get(model, a, LABEL_COLUMN, &desc_a, -1);
  gtk_tree_model_get(model, b, LABEL_COLUMN, &desc_b, -1);
...
static GtkWidget *create_stream_treeview(GvcMixerDialog *dialog,
  ...
  store = gtk_list_store_new(NUM_COLUMNS, G_TYPE_ICON, G_TYPE_STRING,
  ...
  column = gtk_tree_view_column_new_with_attributes(NULL, renderer, "active",
                                                    ACTIVE_COLUMN, NULL);
  ...
  gtk_tree_view_insert_column_with_data_func(
      GTK_TREE_VIEW(treeview), -1, _("Name"), gtk_cell_renderer_text_new(),
      stream_name_to_text, NULL, NULL);

  /* Keep the list of streams sorted by the name */
  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store), LABEL_COLUMN,
                                       GTK_SORT_ASCENDING);

  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(store), LABEL_COLUMN,
                                  compare_stream_treeview_items, NULL, NULL);
  return treeview;
}

static void on_device_profile_changing(GvcComboBox *combobox,
...
static void on_device_selection_changed(GtkTreeSelection *selection,
                                        GvcMixerDialog *dialog) {
  GtkTreeIter iter;
  gchar *name;
  MateMixerDevice *device;
  ...
  gtk_tree_model_get(
      gtk_tree_view_get_model(GTK_TREE_VIEW(dialog->priv->hw_treeview)), &iter,
      HW_NAME_COLUMN, &name, -1);
  ...
  if (profile_switch != NULL) {
    ...
    gtk_box_pack_start(GTK_BOX(dialog->priv->hw_settings_box),
                       dialog->priv->hw_profile_combo, TRUE, TRUE, 6);
...
static void on_notebook_switch_page(GtkNotebook *notebook, GtkWidget *page,
                                    guint page_num, GvcMixerDialog *dialog) {
  MateMixerStreamControl *control;

  // XXX because this is called too early in constructor
  if (G_UNLIKELY(dialog->priv->input_bar == NULL)) return;
...
static void device_name_to_text(GtkTreeViewColumn *column,
                                GtkCellRenderer *cell, GtkTreeModel *model,
                                GtkTreeIter *iter, gpointer user_data) {
  gchar *label = NULL;
  gchar *profile = NULL;
  ...
  gtk_tree_model_get(model, iter, HW_LABEL_COLUMN, &label, HW_PROFILE_COLUMN,
                     &profile, HW_STATUS_COLUMN, &status, -1);

  if (G_LIKELY(status != NULL)) {
    gchar *str;

    if (profile != NULL)
      str = g_strdup_printf("%s\n<i>%s</i>\n<i>%s</i>", label, status, profile);
    else
      str = g_strdup_printf("%s\n<i>%s</i>", label, status);
...
static gint compare_device_treeview_items(GtkTreeModel *model, GtkTreeIter *a,
                                          GtkTreeIter *b, gpointer user_data) {
  gchar *desc_a = NULL;
  gchar *desc_b = NULL;
  gint result;

  gtk_tree_model_get(model, a, HW_LABEL_COLUMN, &desc_a, -1);
  gtk_tree_model_get(model, b, HW_LABEL_COLUMN, &desc_b, -1);
...
static GtkWidget *create_device_treeview(GvcMixerDialog *dialog,
  ...
  column = gtk_tree_view_column_new_with_attributes(NULL, renderer, "gicon",
                                                    HW_ICON_COLUMN, NULL);

  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview), column);
  gtk_tree_view_insert_column_with_data_func(
      GTK_TREE_VIEW(treeview), -1, _("Name"), gtk_cell_renderer_text_new(),
      device_name_to_text, NULL, NULL);

  /* Keep the list of streams sorted by the name */
  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store),
                                       HW_LABEL_COLUMN, GTK_SORT_ASCENDING);

  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(store), HW_LABEL_COLUMN,
                                  compare_device_treeview_items, NULL, NULL);
  return treeview;
}

static void dialog_accel_cb(GtkAccelGroup *accelgroup, GObject *object,
...
static void create_page_effects(GvcMixerDialog *self) {
  GtkWidget *box;
  GtkWidget *label;
  GtkWidget *chooser;
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(self->priv->notebook), box, label);
  ...
  gtk_box_pack_start(GTK_BOX(box), chooser, TRUE, TRUE, 6);
}
...
static GObject *gvc_mixer_dialog_constructor(
  ...
  object = G_OBJECT_CLASS(gvc_mixer_dialog_parent_class)
  ...
  gtk_box_pack_start(GTK_BOX(main_vbox), self->priv->output_stream_box, FALSE,
                     FALSE, 0);
  ...
  for (i = 0; i < G_N_ELEMENTS(tab_accel_keys); i++) {
    closure = g_cclosure_new(G_CALLBACK(dialog_accel_cb), self, NULL);
  ...
  self->priv->hw_treeview =
      create_device_treeview(self, G_CALLBACK(on_device_selection_changed));
  ...
  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(self->priv->hw_treeview));
  ...
  gtk_box_pack_start(GTK_BOX(self->priv->input_box), box, FALSE, FALSE, 6);
  ...
  gtk_label_set_mnemonic_widget(GTK_LABEL(label), self->priv->input_level_bar);
  gtk_widget_set_can_focus(self->priv->input_level_bar, TRUE);
  gtk_box_pack_start(GTK_BOX(box), self->priv->input_level_bar, TRUE, TRUE, 6);
  ...
  gtk_box_pack_start(GTK_BOX(self->priv->input_box),
                     self->priv->input_settings_box, FALSE, FALSE, 0);
  ...
  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(self->priv->input_treeview));
  gtk_tree_selection_set_mode(selection, GTK_SELECTION_SINGLE);
  ...
  self->priv->output_treeview =
      create_stream_treeview(self, G_CALLBACK(on_output_radio_toggled));
  ...
  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(self->priv->output_treeview));
  gtk_tree_selection_set_mode(selection, GTK_SELECTION_SINGLE);
  ...
  gtk_scrolled_window_set_policy(
      GTK_SCROLLED_WINDOW(self->priv->applications_window), GTK_POLICY_NEVER,
      GTK_POLICY_AUTOMATIC);

  gtk_scrolled_window_set_shadow_type(
      GTK_SCROLLED_WINDOW(self->priv->applications_window), GTK_SHADOW_NONE);

  self->priv->applications_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 12);
  gtk_container_set_border_width(GTK_CONTAINER(self->priv->applications_box),
                                 12);
...
static MateMixerContext *gvc_mixer_dialog_get_context(GvcMixerDialog *dialog) {
  return dialog->priv->context;
}

static void gvc_mixer_dialog_set_context(GvcMixerDialog *dialog,
                                         MateMixerContext *context) {
...
static void gvc_mixer_dialog_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
...
static void gvc_mixer_dialog_get_property(GObject *object, guint prop_id,
...
static void gvc_mixer_dialog_dispose(GObject *object) {
...
static void gvc_mixer_dialog_class_init(GvcMixerDialogClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "context", "Context", "MateMixer context", MATE_MIXER_TYPE_CONTEXT,
...
GvcMixerDialog *gvc_mixer_dialog_new(MateMixerContext *context) {
  return g_object_new(GVC_TYPE_MIXER_DIALOG, "icon-name",
                      "multimedia-volume-control", "title",
                      _("Sound Preferences"), "context", context, NULL);
}

gboolean gvc_mixer_dialog_set_page(GvcMixerDialog *self, const gchar *page) {
  guint num = 0;
