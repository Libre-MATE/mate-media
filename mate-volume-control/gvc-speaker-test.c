...
enum { PROP_0, PROP_STREAM, N_PROPERTIES };
...
static void gvc_speaker_test_set_stream(GvcSpeakerTest *test,
                                        MateMixerStream *stream) {
  ...
  for (i = 0; i < G_N_ELEMENTS(positions); i++) {
    gboolean has_position = mate_mixer_stream_control_has_channel_position(
        control, positions[i].position);
...
static void gvc_speaker_test_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
...
static void gvc_speaker_test_get_property(GObject *object, guint prop_id,
...
static void gvc_speaker_test_class_init(GvcSpeakerTestClass *klass) {
  ...
  properties[PROP_STREAM] = g_param_spec_object(
      "stream", "Stream", "MateMixer stream", MATE_MIXER_TYPE_STREAM,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

  g_object_class_install_properties(object_class, N_PROPERTIES, properties);
}

static const gchar *sound_name(MateMixerChannelPosition position) {
  switch (position) {
...
static const gchar *icon_name(MateMixerChannelPosition position,
                              gboolean playing) {
  switch (position) {
    case MATE_MIXER_CHANNEL_FRONT_LEFT:
    ...
    case MATE_MIXER_CHANNEL_BACK_LEFT:
      return playing ? "audio-speaker-left-back-testing"
                     : "audio-speaker-left-back";
    case MATE_MIXER_CHANNEL_BACK_RIGHT:
      return playing ? "audio-speaker-right-back-testing"
                     : "audio-speaker-right-back";
    case MATE_MIXER_CHANNEL_BACK_CENTER:
      return playing ? "audio-speaker-center-back-testing"
                     : "audio-speaker-center-back";
    case MATE_MIXER_CHANNEL_LFE:
      return playing ? "audio-subwoofer-testing" : "audio-subwoofer";
    case MATE_MIXER_CHANNEL_SIDE_LEFT:
      return playing ? "audio-speaker-left-side-testing"
                     : "audio-speaker-left-side";
    case MATE_MIXER_CHANNEL_SIDE_RIGHT:
      return playing ? "audio-speaker-right-side-testing"
...
static void update_button(GtkWidget *control) {
  GtkWidget *button;
  GtkWidget *image;
  gboolean playing;
  MateMixerChannelPosition position;
  ...
  gtk_image_set_from_icon_name(GTK_IMAGE(image), icon_name(position, playing),
                               GTK_ICON_SIZE_DIALOG);
}

static gboolean idle_cb(GtkWidget *control) {
  if (control != NULL) {
...
static void finish_cb(ca_context *c, uint32_t id, int error_code,
                      void *userdata) {
  GtkWidget *control = (GtkWidget *)userdata;

  if (error_code == CA_ERROR_DESTROYED || control == NULL) return;

  g_idle_add((GSourceFunc)idle_cb, control);
}

static void on_test_button_clicked(GtkButton *button, GtkWidget *control) {
  gboolean playing;
  ca_context *canberra;
  ...
  if (playing) {
    ...
    position =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(control), "position"));
    ...
    if (name != NULL) {
      ...
      playing =
          ca_context_play_full(canberra, 1, proplist, finish_cb, control) >= 0;
    }
...
static GtkWidget *create_control(ca_context *canberra,
                                 MateMixerChannelPosition position) {
  GtkWidget *control;
  GtkWidget *box;
  GtkWidget *label;
  ...
  if (name == NULL) name = "audio-volume-medium";
...
static void create_controls(GvcSpeakerTest *test) {
  guint i;

  for (i = 0; i < G_N_ELEMENTS(positions); i++) {
    GtkWidget *control =
        create_control(test->priv->canberra, positions[i].position);

    gtk_grid_attach(GTK_GRID(test), control, positions[i].left,
                    positions[i].top, 1, 1);
...
static void gvc_speaker_test_init(GvcSpeakerTest *test) {
  ...
  gtk_grid_attach(GTK_GRID(test), face, 1, 1, 3, 1);
  ...
  ca_context_change_props(test->priv->canberra, CA_PROP_APPLICATION_ID,
                          "org.mate.VolumeControl", CA_PROP_APPLICATION_NAME,
                          _("Volume Control"), CA_PROP_APPLICATION_VERSION,
                          VERSION, CA_PROP_APPLICATION_ICON_NAME,
...
GtkWidget *gvc_speaker_test_new(MateMixerStream *stream) {
  ...
  test = g_object_new(GVC_TYPE_SPEAKER_TEST, "row-spacing", 6, "column-spacing",
                      6, "row-homogeneous", TRUE, "column-homogeneous", TRUE,
                      "stream", stream, NULL);
