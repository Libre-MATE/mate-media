...
const gchar *gvc_channel_position_to_pulse_string(
  ...
  g_return_val_if_fail(position >= 0 && position < MATE_MIXER_CHANNEL_MAX,
                       NULL);
...
#define HAS_POSITION(p) \
  (mate_mixer_stream_control_has_channel_position(control, (p)))

  /* Modeled after PulseAudio 5.0, probably could be extended with other
   * combinations */
  switch (mate_mixer_stream_control_get_num_channels(control)) {
    case 1:
      if (HAS_POSITION(MATE_MIXER_CHANNEL_MONO)) return _("Mono");
      break;
    case 2:
    ...
    case 5:
      if (HAS_POSITION(MATE_MIXER_CHANNEL_FRONT_LEFT) &&
          ...
          HAS_POSITION(MATE_MIXER_CHANNEL_BACK_LEFT) &&
          HAS_POSITION(MATE_MIXER_CHANNEL_BACK_RIGHT)) {
        if (HAS_POSITION(MATE_MIXER_CHANNEL_LFE)) return _("Surround 4.1");
