...
enum { PROP_0, PROP_CONTROL, PROP_BALANCE_TYPE, N_PROPERTIES };
...
static void create_scale_box(GvcBalanceBar *bar) {
  ...
  bar->priv->scale =
      gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, bar->priv->adjustment);
  ...
  gtk_box_pack_start(GTK_BOX(bar->priv->scale_box), bar->priv->start_box, FALSE,
                     FALSE, 0);
...
static void update_scale_marks(GvcBalanceBar *bar) {
  gchar *str_lower = NULL, *str_upper = NULL;
  gdouble lower, upper;
  ...
  switch (bar->priv->btype) {
    ...
    case BALANCE_TYPE_LFE:
      str_lower =
          g_strdup_printf("<small>%s</small>", C_("balance", "Minimum"));
      str_upper =
          g_strdup_printf("<small>%s</small>", C_("balance", "Maximum"));
      break;
  }

  lower = gtk_adjustment_get_lower(bar->priv->adjustment);
  gtk_scale_add_mark(GTK_SCALE(bar->priv->scale), lower, GTK_POS_BOTTOM,
                     str_lower);
  upper = gtk_adjustment_get_upper(bar->priv->adjustment);
  gtk_scale_add_mark(GTK_SCALE(bar->priv->scale), upper, GTK_POS_BOTTOM,
                     str_upper);
  g_free(str_lower);
  g_free(str_upper);

  if (bar->priv->btype != BALANCE_TYPE_LFE)
    gtk_scale_add_mark(GTK_SCALE(bar->priv->scale), (upper - lower) / 2 + lower,
                       GTK_POS_BOTTOM, NULL);
}
...
static void update_balance_value(GvcBalanceBar *bar) {
  gdouble value = 0;
  ...
  switch (bar->priv->btype) {
    ...
    case BALANCE_TYPE_FR:
      ...
      break;
    case BALANCE_TYPE_LFE:
      value = mate_mixer_stream_control_get_channel_volume(
...
static void on_balance_value_changed(MateMixerStream *stream, GParamSpec *pspec,
                                     GvcBalanceBar *bar) {
  update_balance_value(bar);
}

static gint find_stream_lfe_channel(MateMixerStreamControl *control) {
  guint i;
  ...
  for (i = 0; i < mate_mixer_stream_control_get_num_channels(control); i++) {
    ...
    if (position == MATE_MIXER_CHANNEL_LFE) return i;
  }

  return -1;
}

static void gvc_balance_bar_set_control(GvcBalanceBar *bar,
                                        MateMixerStreamControl *control) {
...
static void gvc_balance_bar_set_balance_type(GvcBalanceBar *bar,
                                             GvcBalanceType btype) {
  GtkWidget *frame;
  GtkAdjustment *adjustment;
   ...
   * stream is set. */
  adjustment =
      GTK_ADJUSTMENT(gtk_adjustment_new(0.0, -1.0, 1.0, 0.05, 0.5, 0.0));
  ...
  gtk_label_set_mnemonic_widget(GTK_LABEL(bar->priv->label), bar->priv->scale);
...
static void gvc_balance_bar_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
...
static void gvc_balance_bar_get_property(GObject *object, guint prop_id,
...
static void gvc_balance_bar_class_init(GvcBalanceBarClass *klass) {
  ...
  properties[PROP_CONTROL] =
      g_param_spec_object("control", "Control", "MateMixer stream control",
                          MATE_MIXER_TYPE_STREAM_CONTROL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  properties[PROP_BALANCE_TYPE] = g_param_spec_int(
      "balance-type", "balance type",
      "Whether the balance is right-left or front-rear", BALANCE_TYPE_RL,
      NUM_BALANCE_TYPES - 1, BALANCE_TYPE_RL,
...
static void on_adjustment_value_changed(GtkAdjustment *adjustment,
  ...
  if (bar->priv->control == NULL) return;

  value = gtk_adjustment_get_value(adjustment);

  switch (bar->priv->btype) {
    case BALANCE_TYPE_RL:
      mate_mixer_stream_control_set_balance(bar->priv->control, (gfloat)value);
      break;
    case BALANCE_TYPE_FR:
      mate_mixer_stream_control_set_fade(bar->priv->control, (gfloat)value);
      break;
    case BALANCE_TYPE_LFE:
      mate_mixer_stream_control_set_channel_volume(
          bar->priv->control, bar->priv->lfe_channel, (guint)value);
      break;
  }
}

static void gvc_balance_bar_init(GvcBalanceBar *bar) {
...
GtkWidget *gvc_balance_bar_new(MateMixerStreamControl *control,
                               GvcBalanceType btype) {
  return g_object_new(GVC_TYPE_BALANCE_BAR, "balance-type", btype, "control",
                      control, "orientation", GTK_ORIENTATION_HORIZONTAL, NULL);
}
