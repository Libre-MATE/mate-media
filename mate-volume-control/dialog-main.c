...
static void on_dialog_response(GtkDialog *dialog, guint response_id,
                               gpointer data) {
  gboolean destroy = GPOINTER_TO_INT(data);

  if (destroy == TRUE) gtk_widget_destroy(GTK_WIDGET(dialog));

  gtk_main_quit();
}

static void on_dialog_close(GtkDialog *dialog, gpointer data) {
  gboolean destroy = GPOINTER_TO_INT(data);
...
static void remove_warning_dialog(void) {
  if (popup_id != 0) {
...
static void context_ready(MateMixerContext *context,
...
static void on_context_state_notify(MateMixerContext *context,
                                    GParamSpec *pspec, GtkApplication *app) {
  ...
  if (state == MATE_MIXER_STATE_READY) {
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(app_dialog), 0,
...
static gboolean dialog_popup_timeout(gpointer data) {
...
int main(int argc, char **argv) {
  ...
  GOptionEntry entries[] = {
      {"backend", 'b', 0, G_OPTION_ARG_STRING, &backend,
       N_("Sound system backend"), "pulse|alsa|oss|null"},
      {"debug", 'd', 0, G_OPTION_ARG_NONE, &debug, N_("Enable debug"), NULL},
      {"page", 'p', 0, G_OPTION_ARG_STRING, &page, N_("Startup page"),
       "effects|hardware|input|output|applications"},
  ...
  gtk_init_with_args(&argc, &argv, _(" — MATE Volume Control"), entries,
  ...
  if (mate_mixer_context_get_state(context) == MATE_MIXER_STATE_CONNECTING) {
    popup_id =
        g_timeout_add_seconds(DIALOG_POPUP_TIMEOUT, dialog_popup_timeout, NULL);
  }
