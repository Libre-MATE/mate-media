...
enum { PROP_0, PROP_CONTROL, PROP_DISPLAY_NAME, PROP_ICON_NAMES, N_PROPERTIES };
...
G_DEFINE_TYPE_WITH_PRIVATE(GvcStreamAppletIcon, gvc_stream_applet_icon,
                           GTK_TYPE_EVENT_BOX)

static gboolean popup_dock(GvcStreamAppletIcon *icon, guint time) {
  ...
  gdk_window_get_origin(gtk_widget_get_window(GTK_WIDGET(icon)), &allocation.x,
                        &allocation.y);
  ...
  monitor_num = gdk_display_get_monitor_at_point(gdk_screen_get_display(screen),
  ...
  if (icon->priv->orient == MATE_PANEL_APPLET_ORIENT_LEFT ||
    ...
    if (allocation.x + allocation.width + dock_req.width <=
    ...
    if (allocation.y + allocation.height + dock_req.height <=
  ...
  do {
    ...
    if (gdk_seat_grab(seat, window, GDK_SEAT_CAPABILITY_ALL, TRUE, NULL, NULL,
...
static gboolean on_applet_icon_button_press(GtkWidget *applet_icon,
...
static gboolean on_applet_icon_scroll_event(GtkWidget *event_box,
                                            GdkEventScroll *event,
                                            GvcStreamAppletIcon *icon) {
  return gvc_channel_bar_scroll(GVC_CHANNEL_BAR(icon->priv->bar),
                                event->direction);
}

static void gvc_icon_release_grab(GvcStreamAppletIcon *icon,
                                  GdkEventButton *event) {
...
static gboolean on_dock_button_press(GtkWidget *widget, GdkEventButton *event,
                                     GvcStreamAppletIcon *icon) {
...
static void popdown_dock(GvcStreamAppletIcon *icon) {
  GdkDisplay *display;
...
static void gvc_icon_grab_notify(GvcStreamAppletIcon *icon,
                                 gboolean was_grabbed) {
  if (was_grabbed != FALSE) return;

  if (gtk_widget_has_grab(icon->priv->dock) == FALSE) return;
...
static void on_dock_grab_notify(GtkWidget *widget, gboolean was_grabbed,
                                GvcStreamAppletIcon *icon) {
  gvc_icon_grab_notify(icon, was_grabbed);
}

static gboolean on_dock_grab_broken_event(GtkWidget *widget,
                                          gboolean was_grabbed,
                                          GvcStreamAppletIcon *icon) {
...
static gboolean on_dock_key_release(GtkWidget *widget, GdkEventKey *event,
                                    GvcStreamAppletIcon *icon) {
...
static gboolean on_dock_scroll_event(GtkWidget *widget, GdkEventScroll *event,
                                     GvcStreamAppletIcon *icon) {
...
static void gvc_stream_applet_icon_set_icon_from_name(GvcStreamAppletIcon *icon,
  ...
  cairo_surface_t *surface = gtk_icon_theme_load_surface(
      icon_theme, icon_name, icon->priv->size, icon_scale, NULL,
    ...
    if (flags & MATE_MIXER_STREAM_CONTROL_HAS_DECIBEL) {
      if (decibel > -MATE_MIXER_INFINITY) {
        markup = g_strdup_printf(
            "<b>%s: %u%%</b>\n"
            "<small>%0.2f dB\n%s</small>",
            icon->priv->display_name, volume_percent, decibel, description);
      } else {
        markup = g_strdup_printf(
            "<b>%s: %u%%</b>\n"
            "<small>-&#8734; dB\n%s</small>",
            icon->priv->display_name, volume_percent, description);
      }
    } else {
      markup = g_strdup_printf("<b>%s: %u%%</b>\n<small>%s</small>",
                               icon->priv->display_name, volume_percent,
                               description);
    }
  } else {
    markup = g_strdup_printf("<b>%s</b>\n<small>%s</small>",
                             icon->priv->display_name, description);
  }
...
void gvc_stream_applet_icon_set_size(GvcStreamAppletIcon *icon, guint size) {
  ...
  icon->priv->size = size;
  gvc_stream_applet_icon_set_icon_from_name(
      icon, icon->priv->icon_names[icon->priv->current_icon]);
}

void gvc_stream_applet_icon_set_orient(GvcStreamAppletIcon *icon,
                                       MatePanelAppletOrient orient) {
  /* Sometimes orient does not get properly defined especially on a bottom
   * panel. Use the applet orientation if it is valid, otherwise set a vertical
   * slider, otherwise bottom panels get a horizontal slider.
   */
  if (orient)
...
static void on_stream_control_volume_notify(MateMixerStreamControl *control,
                                            GParamSpec *pspec,
                                            GvcStreamAppletIcon *icon) {
  update_icon(icon);
}
...
void gvc_stream_applet_icon_set_control(GvcStreamAppletIcon *icon,
  ...
  if (icon->priv->control == control) return;

  if (control != NULL) g_object_ref(control);

  if (icon->priv->control != NULL) {
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(icon->priv->control),
  ...
  gvc_channel_bar_set_control(GVC_CHANNEL_BAR(icon->priv->bar),
                              icon->priv->control);

  g_object_notify_by_pspec(G_OBJECT(icon), properties[PROP_CONTROL]);
}

static void gvc_stream_applet_icon_set_property(GObject *object, guint prop_id,
                                                const GValue *value,
                                                GParamSpec *pspec) {
...
static void gvc_stream_applet_icon_get_property(GObject *object, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
  GvcStreamAppletIcon *self = GVC_STREAM_APPLET_ICON(object);
...
static void gvc_stream_applet_icon_dispose(GObject *object) {
...
static void gvc_stream_applet_icon_class_init(GvcStreamAppletIconClass *klass) {
  ...
  properties[PROP_DISPLAY_NAME] = g_param_spec_string(
      "display-name", "Display name", "Name to display for this stream", NULL,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

  properties[PROP_ICON_NAMES] = g_param_spec_boxed(
      "icon-names", "Icon names", "Name of icon to display for this stream",
      G_TYPE_STRV,
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);
...
static void on_applet_icon_visible_notify(GvcStreamAppletIcon *icon) {
...
static void gvc_stream_applet_icon_init(GvcStreamAppletIcon *icon) {
  GtkWidget *frame;
  GtkWidget *box;
  ...
  g_signal_connect(gtk_settings_get_default(), "notify::gtk-icon-theme-name",
                   G_CALLBACK(on_icon_theme_change), icon);
}
...
GvcStreamAppletIcon *gvc_stream_applet_icon_new(MateMixerStreamControl *control,
  ...
  return g_object_new(GVC_TYPE_STREAM_APPLET_ICON, "control", control,
                      "icon-names", icon_names, NULL);
}
