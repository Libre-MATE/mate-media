...
static const gchar *icon_names_input[] = {
    "audio-input-microphone-muted", "audio-input-microphone-low",
    "audio-input-microphone-medium", "audio-input-microphone-high", NULL};

static void menu_output_mute(GtkAction *action, GvcApplet *applet);
static void menu_activate_open_volume_control(GtkAction *action,
                                              GvcApplet *applet);
static const GtkActionEntry applet_menu_actions[] = {
    {"Preferences", APPLET_ICON, N_("_Sound Preferences"), NULL, NULL,
     G_CALLBACK(menu_activate_open_volume_control)},
    {"MuteOutput", "audio-volume-muted", N_("Mute Output"), NULL, NULL,
     G_CALLBACK(menu_output_mute)}};
...
static void update_icon_input(GvcApplet *applet) {
    ...
    while (inputs != NULL) {
      ...
      if (role == MATE_MIXER_STREAM_CONTROL_ROLE_APPLICATION) {
        ...
        if (strcmp(app_id, "org.mate.VolumeControl") != 0 &&
          ...
          if (G_UNLIKELY(control == NULL)) control = input;
...
static void update_icon_output(GvcApplet *applet) {
...
static void on_output_stream_control_added(MateMixerStream *stream,
                                           const gchar *name,
                                           GvcApplet *applet) {
  ...
  if (G_LIKELY(control != NULL)) {
    ...
    if (role != MATE_MIXER_STREAM_CONTROL_ROLE_APPLICATION) return;
  }
...
static void on_input_stream_control_added(MateMixerStream *stream,
                                          const gchar *name,
                                          GvcApplet *applet) {
  MateMixerStreamControl *control;
...
static void on_output_stream_control_removed(MateMixerStream *stream,
                                             const gchar *name,
                                             GvcApplet *applet) {
...
static void on_input_stream_control_removed(MateMixerStream *stream,
                                            const gchar *name,
                                            GvcApplet *applet) {
...
static gboolean update_default_output_stream(GvcApplet *applet) {
  MateMixerStream *stream;

  stream = mate_mixer_context_get_default_output_stream(applet->priv->context);
  if (stream == applet->priv->output) return FALSE;

  /* The output stream has changed */
  if (applet->priv->output != NULL) {
    g_signal_handlers_disconnect_by_data(G_OBJECT(applet->priv->output),
                                         applet);
...
static gboolean update_default_input_stream(GvcApplet *applet) {
  MateMixerStream *stream;

  stream = mate_mixer_context_get_default_input_stream(applet->priv->context);
  if (stream == applet->priv->input) return FALSE;
...
static void on_context_state_notify(MateMixerContext *context,
                                    GParamSpec *pspec, GvcApplet *applet) {
...
static void on_context_default_input_stream_notify(MateMixerContext *context,
                                                   GParamSpec *pspec,
                                                   GvcApplet *applet) {
  if (update_default_input_stream(applet) == FALSE) return;

  update_icon_input(applet);
}

static void on_context_default_output_stream_notify(MateMixerContext *control,
                                                    GParamSpec *pspec,
                                                    GvcApplet *applet) {
  if (update_default_output_stream(applet) == FALSE) return;
...
void gvc_applet_start(GvcApplet *applet) {
  ...
  if (G_UNLIKELY(applet->priv->running == TRUE)) return;
...
static void gvc_applet_dispose(GObject *object) {
...
static void gvc_applet_init(GvcApplet *applet) {
  ...
  applet->priv->icon_output =
      gvc_stream_applet_icon_new(NULL, icon_names_output);

  gvc_stream_applet_icon_set_display_name(applet->priv->icon_input, _("Input"));
  gvc_stream_applet_icon_set_display_name(applet->priv->icon_output,
                                          _("Output"));
...
GvcApplet *gvc_applet_new(void) { return g_object_new(GVC_TYPE_APPLET, NULL); }

static void gvc_applet_set_size(GtkWidget *widget, int size,
                                gpointer user_data) {
...
static void gvc_applet_set_mute(GtkWidget *widget, int size,
                                gpointer user_data) {
  GvcApplet *applet = user_data;
  gboolean is_muted;
  GtkAction *action;
  ...
  action =
      gtk_action_group_get_action(applet->priv->action_group, "MuteOutput");
...
static void gvc_applet_set_orient(GtkWidget *widget,
...
gboolean gvc_applet_fill(GvcApplet *applet, MatePanelApplet *applet_widget) {
  ...
  /*FIXME: We haved to set this up BEFORE packing in icons. find a way to update
   * this when the applet is moved that works*/
  ...
  gvc_stream_applet_icon_set_size(
      applet->priv->icon_input,
      mate_panel_applet_get_size(applet->priv->applet));
  gvc_stream_applet_icon_set_size(
      applet->priv->icon_output,
      mate_panel_applet_get_size(applet->priv->applet));
  gvc_stream_applet_icon_set_orient(
      applet->priv->icon_input,
      mate_panel_applet_get_orient(applet->priv->applet));
  gvc_stream_applet_icon_set_orient(
      applet->priv->icon_output,
      mate_panel_applet_get_orient(applet->priv->applet));

  /* we add the Gtk buttons into the applet */
  gtk_box_pack_start(applet->priv->box, GTK_WIDGET(applet->priv->icon_input),
                     TRUE, TRUE, 2);
  gtk_box_pack_start(applet->priv->box, GTK_WIDGET(applet->priv->icon_output),
                     TRUE, TRUE, 2);
  gtk_container_add(GTK_CONTAINER(applet->priv->applet),
                    GTK_WIDGET(applet->priv->box));
  ...
  g_object_connect(applet->priv->applet, "signal::change_size",
                   gvc_applet_set_size, applet, "signal::change_orient",
                   gvc_applet_set_orient, applet, "signal::event-after",
                   gvc_applet_set_mute, applet, NULL);

  /* set up context menu */
  applet->priv->action_group =
      gtk_action_group_new("Volume Control Applet Actions");
  gtk_action_group_set_translation_domain(applet->priv->action_group,
                                          GETTEXT_PACKAGE);
  gtk_action_group_add_actions(applet->priv->action_group, applet_menu_actions,
                               G_N_ELEMENTS(applet_menu_actions), applet);

  mate_panel_applet_setup_menu(applet->priv->applet, ui,
                               applet->priv->action_group);
